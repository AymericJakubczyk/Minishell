...
/*   Updated: 2023/07/24 17:34:41 by cprojean         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_void(t_entry *entry, int *ind)
{
	...
	stock_ind = *ind + 2;
	if (*ind - 1 < 0 || (entry[*ind - 1].type == WHITESPACE && \
		entry[*ind - 1].context == NO_QUOTE))
	{
		if (!entry[*ind + 2].c || (entry[*ind + 2].type == WHITESPACE \
			&& entry[*ind + 2].context == NO_QUOTE))
			return (*ind += 2, 1);
		else
		{
			while (entry[stock_ind].c && (entry[stock_ind].type == S_QUOTE || \
					entry[stock_ind].type == D_QUOTE) && \
					entry[stock_ind].context == NO_QUOTE)
				stock_ind++;
			if (!entry[stock_ind].c || (entry[stock_ind].type == WHITESPACE \
				&& entry[stock_ind].context == NO_QUOTE))
